...
namespace OpenRT
{
    ...
    /// 
    /// </summary>    
    public class RTMeshBVHDynamic : IRTMeshBVH
    {
        public const int TRIANGLE_STRIDE = 24; // 3 * float3 vertices + 3 * float3 normals + 3 * float2 UVs
        private const bool OVERRIDE_NEVER_DIRTY = false; //stops this object from getting refreshed in the renderer's eyes when moving. will cause object to not update
        ...
        {
            if (OVERRIDE_NEVER_DIRTY)
            {
                return false; //just causes the position of the object to never update in the renderer, which makes sense, the geo buffer will not change like this.
            }
